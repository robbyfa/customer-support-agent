...
**Model:**
- [ ] Create `models/classification.py` — `TicketClassification(BaseModel)` with category (7 Literal types), urgency, sentiment, summary, requires_human_review, sensitive_case, extracted_customer_id
- [ ] Update `models/__init__.py` — export TicketClassification

**Chain:**
- [ ] Create `graph/chains/classifier.py` — `classification_chain = prompt | llm.with_structured_output(TicketClassification)`
- [ ] Update `graph/chains/__init__.py`

**Node:**
- [ ] Create `graph/nodes/classify_ticket.py` — `def classify_ticket(state) -> dict` returning classification, customer_id, audit_trail
- [ ] Update `graph/nodes/__init__.py` — export classify_ticket

**Tests:**
- [ ] Add schema tests in `tests/test_schemas.py` — instantiation, Literal validation, optional fields, serialization
- [ ] Add chain/node tests in `tests/test_graph.py` — "failed withdrawal" → withdrawal_issue/high/negative, "can't log in" → login_issue
- [ ] Verify: `uv run pytest tests/test_schemas.py tests/test_graph.py -v` passes
